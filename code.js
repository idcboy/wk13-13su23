...
function setTime(){
    ...
    //2. use this number to create timers time for a loop
    var currTime = numOfSec;
    for(var i = 0; i <= numOfSec; i++ ){
        if(i == numOfSec){
            setTimeout(function(){
                document.getElementById("statusUpdate").innerHTML = "Time for heist music";
                currTime--;
            },i * 1000);
        } else {
        setTimeout(function(){
    ...
    //3. use number to play music after number of secoonds hits 0
    setTimeout(function(){
        playSneak();
    }, numOfSec * 1000 )
}

//function to add a differnt sound for count down
function playSneak(){
    console.log("playSneak() started");
    mySound = new sound("sneak.wav");
    mySound.play();
}
